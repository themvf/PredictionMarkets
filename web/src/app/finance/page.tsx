...
import {
  getFinanceStats,
  getFinanceSubcategoryCounts,
  getFinanceWhaleTrades,
  getFinanceTopTraders,
  getFinanceAnomalies,
  type FinanceStats,
...
function StatsHero({ stats }: { stats: FinanceStats }) {
  const cards = [
    {
...
async function WhaleTab({ subcategory }: { subcategory?: string }) {
  const trades = await getFinanceWhaleTrades(subcategory || undefined, 50);
...
async function SharpMoneyTab() {
  const [topTraders, anomalies] = await Promise.all([
    getFinanceTopTraders(20),
    getFinanceAnomalies(10),
  ]);
...
async function FinanceContent({ searchParams }: Props) {
  ...
  let subcategoryCounts: Awaited<ReturnType<typeof getFinanceSubcategoryCounts>> = [];
  let stats: FinanceStats = { totalMarkets: 0, totalVolume: 0, whaleTradeCount: 0 };
  try {
